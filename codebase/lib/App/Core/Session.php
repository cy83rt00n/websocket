...
class Session
{
    ...
    private static function session_common()
    {
        ...
        $row = DB::fetch_row($q);
        if (!$row) {
            return Session::unset_cookie_token();
        }
        // vars
    ...
    public static function auth_send($d)
    {
        ...
        // validate
        if (!$phone) {
            return error_response(1003, 'One of the parameters was missing or was passed in the wrong format.', ['phone' => 'empty field']);
        }
        $user = User::user_info(['phone' => $phone]);
        if ($user['access'] != 1) {
            return error_response(1004, 'User with this phone is not found.', ['phone' => 'incorrect phone']);
        }
        // query
    ...
    public static function auth_confirm($d)
    {
        ...
        // error (empty)
        if (!$phone && !$code) {
            return error_response(1003, 'One of the parameters was missing or was passed in the wrong format.', ['phone' => 'empty field', 'code' => 'empty field']);
        }
        if (!$phone) {
            return error_response(1003, 'One of the parameters was missing or was passed in the wrong format.', ['phone' => 'empty field']);
        }
        if (!$code) {
            return error_response(1003, 'One of the parameters was missing or was passed in the wrong format.', ['code' => 'empty field']);
        }
        // check
        ...
        // error (unregistered)
        if (!$row) {
            return error_response(1004, 'User with this phone is not found', ['phone' => 'user is not registered']);
        }
        ...
        if (!$attempts) {
            return error_response(1005, 'Number of invalid code attempts has been exceeded for this user, please try again later.', ['code' => 'exceeded error limit, please try later']);
        }
        // error (code)
    ...
    {
        if (Session::$token) {
            setcookie('token', Session::$token, strtotime('+1 year'), '/', '', Session::$cookies_secure, true);
        }
