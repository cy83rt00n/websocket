...
class Compiler extends HTML
{
    ...
    private function compile_tag($tag)
    {
        ...
        $args = $match[2] ?? null;
        // type
        if (preg_match('!^'.$this->var_regexp.'$!', $command)) {
            return "<?php echo ".$this->parse_var_props($command)."; ?>\n";
        } elseif (preg_match('/^LANG_[a-z0-9_]+$/i', $command)) {
            return "<?php echo self::\$_langs['".$command."']; ?>\n";
        } elseif ($command == 'include') {
            return $this->compile_include_tag($args);
        } elseif ($command == 'if') {
            return $this->compile_if_tag($args);
        } elseif ($command == 'else') {
            return '<?php else: ?>';
        } elseif ($command == 'elseif') {
            return $this->compile_if_tag($args, true);
        } elseif ($command == '/if') {
            return '<?php endif; ?>';
        } elseif ($command == 'section') {
            return $this->compile_section($args);
        } elseif ($command == '/section') {
            return "<?php endfor; endif; ?>";
        } elseif ($command == 'literal') {
            return $this->compile_literal();
        } else {
            $this->syntax_error("wrong tag '".$command."'", E_USER_ERROR, __FILE__, __LINE__);
        }
    }
    ...
    private function parse_vars_props(&$tokens)
    {
        foreach ($tokens as $key => $val) {
            $tokens[$key] = $this->parse_var_props($val);
        }
    }
    ...
    private function parse_var_props($val)
    {
        ...
        } elseif (preg_match('!^'.$this->db_qstr_regexp.'$!', $val)) {
            preg_match('!^('.$this->db_qstr_regexp.')$!', $val, $match);
            return $this->expand_quoted_text($match[1]);
        } elseif (preg_match('!^'.$this->si_qstr_regexp.'$!', $val)) {
            return $val;
            ...
            if ($index[0] == '[') {
                ...
                if (is_numeric($index)) {
                    ...
                    if (strpos($index, '.') !== false) {
                        $output .= '['.$this->parse_var($index).']';
                    } else {
                        $output .= "[self::\$_tpl_vars['".substr($index, 1)."']]";
                    }
                } else {
                ...
                if ($index[1] == '$') {
                    $output .= "[self::\$_tpl_vars['".substr($index, 2)."']]";
                } else {
    ...
    private function parse_attrs($args)
    {
            ...
            switch ($state) {
                case 0:
                    if (preg_match('!^\w+$!', $token)) {
                        ...
                        $this->syntax_error("invalid attribute name: '".$token."'", E_USER_ERROR, __FILE__, __LINE__);
                    }
                    break;
                case 1:
                    if ($token == '=') {
                        ...
                        $this->syntax_error("expecting '=' after attribute name '".$token."'", E_USER_ERROR, __FILE__, __LINE__);
                    }
                    break;
                case 2:
                    if ($token != '=') {
                        if (preg_match('!^(on|yes|true)$!', $token)) {
                            ...
                            $token = 'null';
                        } elseif (!preg_match('!^'.$this->var_regexp.'$!', $token)) {
                            $token = '"'.addslashes($token).'"';
                        }
                        $attrs[$attr_name] = $token;
                        $state = 0;
                    } else {
                        $this->syntax_error("'=' cannot be an attribute value", E_USER_ERROR, __FILE__, __LINE__);
                    }
                    break;
            }
            $last_token = $token;
        ...
        if ($state != 0) {
            if ($state == 1) {
                $this->syntax_error("expecting '=' after attribute name '".$last_token."'", E_USER_ERROR, __FILE__, __LINE__);
            } else {
                $this->syntax_error("missing attribute value", E_USER_ERROR, __FILE__, __LINE__);
            }
        }
        // output
    ...
    private function compile_include_tag($args)
    {
        ...
        $arg_list = [];
        if (empty($attrs['file'])) {
            $this->syntax_error("missing 'file' attribute in include tag", E_USER_ERROR, __FILE__, __LINE__);
        }
        // attrs
    ...
    private function compile_if_tag($args, $elseif = false)
    {
        ...
        if (isset($token_count['(']) && $token_count['('] != $token_count[')']) {
            $this->syntax_error('unbalanced parenthesis in if statement', E_USER_ERROR, __FILE__, __LINE__);
        }
        // tokens
        for ($i = 0, $max = count($tokens); $i < $max; $i++) {
            $token = &$tokens[$i];
            if (is_numeric($token) || in_array($token, ['!','%','!==','==','===','>','<','!=','<>','<=','>=','&&','||','|','^','&','~','(',')',',','+','-','*','/']) || preg_match('!^'.$this->func_regexp.'$!', $token)) {
                continue;
            } elseif (preg_match('!^'.$this->var_regexp.'$!', $token)) {
                $token = $this->parse_var_props($token);
            } else {
                $this->syntax_error("unidentified token '".$token."'", E_USER_ERROR, __FILE__, __LINE__);
            }
        }
        // output
    ...
    private function compile_section($args)
    {
        ...
        if (empty($section_name)) {
            $this->syntax_error("missing section name", E_USER_ERROR, __FILE__, __LINE__);
        }
        // parse
        $output = '<?php ';
        ...
        foreach ($attrs as $attr_name => $attr_value) {
            ...
            } elseif ($attr_name == 'name') {
                $output .= "{$section_props}['$attr_name'] = $attr_value;\n";
            } elseif ($attr_name == 'max' || $attr_name == 'start') {
                $output .= "{$section_props}['$attr_name'] = (int)$attr_value;\n";
            } elseif ($attr_name == 'step') {
                $output .= "{$section_props}['$attr_name'] = ((int)$attr_value) == 0 ? 1 : (int)$attr_value;\n";
            } else {
                $this->syntax_error("unknown section attribute - '".$attr_name."'", E_USER_ERROR, __FILE__, __LINE__);
            }
        }
        if (!isset($attrs['show'])) {
    ...
    private function compile_ref(&$indexes)
    {
        ...
        foreach ($indexes as $index_no => $index) {
            if (substr($index, 0, 1) != '.' && $index_no < 2 || !preg_match('!^(\.|\[)!', $index)) {
                $this->syntax_error('$ref'.implode('', array_slice($indexes, 0, 2)).' is an invalid reference', E_USER_ERROR, __FILE__, __LINE__);
            }
        }
        // search
        if ($ref == 'section') {
            ...
            $this->syntax_error('$ref.'.$ref.' is an unknown reference', E_USER_ERROR, __FILE__, __LINE__);
        }
        // validate
        if (isset($max_index) && count($indexes) > $max_index) {
            $this->syntax_error('$ref'.implode('', $indexes).' is an invalid reference', E_USER_ERROR, __FILE__, __LINE__);
        }
        // output
        array_shift($indexes);
        return $compiled_ref;
    ...
    private function expand_quoted_text($var_expr)
    {
        if (preg_match_all('%(?:\`(?<!\\\\)\$'.$this->dvar_guts_regexp.'\`)|(?:(?<!\\\\)\$\w+(\[[a-zA-Z0-9]+\])*)%', $var_expr, $match)) {
            ...
            foreach ($match as $var) {
                $replace[$var] = '".('.$this->parse_var(str_replace('`', '', $var)).')."';
            }
